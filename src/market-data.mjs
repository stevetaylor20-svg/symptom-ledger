...
// --- Upcoming dividends via ONE calendar call, filtered to our symbols ---
async function refreshDividends(idBySymbol) {
  const start = new Date().toISOString().slice(0, 10);
  const end = new Date(Date.now() + 90 * 86400000).toISOString().slice(0, 10);

  // Build a lookup from TD symbol+mic back to our instrument_id.
  const key = (sym, mic) => `${sym}|${mic || ""}`;
  const idByKey = new Map();
  for (const [display, id] of idBySymbol) {
    const m = mapSymbol(display);
    idByKey.set(key(m.td, m.mic), id);
  }

  const cal = await td(`dividends_calendar?start_date=${start}&end_date=${end}`);
  const items = cal.dividends || cal.data || [];

  const rows = [];
  for (const d of items) {
    const mic = d.mic_code || null;
    const id = idByKey.get(key(d.symbol, mic)) ?? idByKey.get(key(d.symbol, null));
    if (!id) continue; // not a symbol we track
